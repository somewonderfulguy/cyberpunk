...
type ReactTabListProps = ComponentProps<typeof ReactTabList>

const TabList = forwardRef<HTMLDivElement, ReactTabListProps & HTMLAttributes<HTMLDivElement>>(
  ({ children, ...props }, ref) => {
    return (
...
const TabListImpl = forwardRef<HTMLDivElement, ReactTabListProps & HTMLAttributes<HTMLDivElement>>(
  ({ children, ...props }, ref) => {
    ...
    const refWrapper = useResizeObserver((bounds) => setContainerWidth(bounds.width), 100)
    ...
    const { indicatorLeft, indicatorWidth } = useIndicatorPosition(tabs, refWrapper, containerWidth)

    const animatedRef = useTrackIndicatorPosition()

    return (
      <div ref={refWrapper} className={styles.tabListContainer}>
        <ReactTabList {...props} ref={ref} {...(isHexagon && { 'data-augmented-ui': 'tl-clip br-clip border' })}>
